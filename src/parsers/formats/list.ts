...
export function boardToMd(board: Board) {
  ...
  // Include board notes before the lanes if they exist
  const boardNotes = board.data.boardNotes ? board.data.boardNotes + '\n\n' : '';

  return frontmatter + boardNotes + lanes + archiveToMd(board.data.archive) + settingsToCodeblock(board);
}
